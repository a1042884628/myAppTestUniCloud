<template>
	<view class="busin-container">
		<skeleton>
			...
			<template slot="input">
				<view class="input-class">
					<input class="uni-input" v-model="form.grand" type="digit" maxlength="10"
		...
		<skeleton>
			...
			<template slot="input">
				<view class="input-class">
					<input class="uni-input" v-model="form.amount" type="digit" maxlength="10"
			...
			<template slot="input">
				<radio-group @change="radioRateChange">
					<label style="margin-right: 30rpx;" v-for="(item, index) in rateArr" :key="item.value">
						<radio style="transform: scale(0.8);color: #2295FF !important;"  :value="item.value" :checked="index === rateCurrent" /><text
...
<script>
	...
	export default {
		...
		methods: {
			...
			pointInput(val) {
				if (this.form.lpr) {
					this.form.rateLpr = (Number(this.form.lpr) + Number(val.detail.value) / 100).toFixed(2)
				} else {
