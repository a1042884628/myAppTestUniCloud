<template>
	<view class="busin-container">
		<skeleton>
			...
			<template slot="input">
				<radio-group @change="radioPriceChange">
					<label style="margin-right: 10rpx;" v-for="(item, index) in items" :key="item.value">
		...
		<skeleton v-if="form.way !== '1'">
			...
			<template slot="input">
				<view class="input-class">
					<input class="uni-input" v-model="form.unit" @input="unitInput" type="digit" maxlength="10"
						placeholder="请输入单价" />
					<view class="">
						元/㎡
					</view>
		...
		<skeleton v-if="form.way !== '1'">
			...
			<template slot="input">
				<view class="input-class">
					<input class="uni-input" v-model="form.area" @input="areaInput" type="digit" maxlength="10"
						placeholder="请输入房屋面积" />
					<view class="">
						㎡
					</view>
			...
			<template slot="input">
				<view class="input-class">
					<input class="uni-input" @input='totalPriceInput' v-model="form.totalPrice"
						:disabled="form.way !== '1'" type="digit" maxlength="10" placeholder="请输入房屋总价" />
					<view class="">
						万元
					</view>
				</view>
			...
			<template slot="input">
				<radio-group @change="radioRateChange">
					<label style="margin-right: 30rpx;" v-for="(item, index) in rateArr" :key="item.value">
						<radio style="transform: scale(0.8);color: #2295FF;" :value="item.value" :checked="index == rateCurrent" /><text
			...
			<view class="right-form">
				<view class="right-form-rate">
					...
					<view class="right-form-rate-right">
						<input class="uni-input" @input='LprInput' v-model="form.lpr" type="digit" maxlength="10" />
						<view class="">
							%
						</view>
					</view>
				...
				<view class="right-form-rate">
					...
					<view class="right-form-rate-right">
						<input class="uni-input" @input='pointInput' v-model="form.point" type="digit"
							maxlength="10" />
						<view class="">
							‱
						</view>
	...
	export default {
		...
		methods: {
			...
			LprInput(val) {
				...
				if (this.form.point) {
					this.form.rateLpr = (Number(this.form.point) / 100 + Number(val.detail.value||0)).toFixed(2)
				} else {
					this.form.rateLpr = (Number(val.detail.value||0)).toFixed(2)
				}
			},
			//  基点
			pointInput(val) {
				
				console.log("val",val,(this.form.lpr + Number(val.detail.value||0) / 100))
				if (this.form.lpr) {
					this.form.rateLpr = (Number(this.form.lpr) + Number(val.detail.value||0) / 100).toFixed(2)
				} else {
					this.form.rateLpr = (Number(val.detail.value||0) / 100).toFixed(2)
				}
			},
			unitInput(val) {
				
				if (this.form.area) {
					this.form.totalPrice = (Number(this.form.area) * Number(val.detail.value||0)) / 10000
					this.form.amount = (Number(this.form.totalPrice) * this.form.percentage).toFixed(2)
				}
			},
			areaInput(val) {
				if (this.form.unit) {
					this.form.totalPrice = (Number(this.form.unit) * Number(val.detail.value||0)) / 10000
					this.form.amount = (Number(this.form.totalPrice) * Number(this.form.percentage)).toFixed(2)
				}
			},
			// 计算方式勾选事件
			...
			radioRateChange(val) {
				...
				for (let i = 0; i < this.rateArr.length; i++) {
				                if (this.rateArr[i].value === val.detail.value) {
				                    this.rateCurrent = i;
				                    break;
				                }
				            }
